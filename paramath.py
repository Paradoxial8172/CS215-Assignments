...
def golden_ratio(n: int) -> float:
    """Uses the Fibonacci Sequence and calculates the golden ratio by taking the last term of the returned Fibonacci Sequence divided by the second to last term.

    Args:
        n (int): Number of terms of the Fibonacci Sequence.

    Returns:
        float: The golden ratio. (Only an aproximation; use more terms to get a more accurate result.)
    """

    if n > 100000:
        return "Calculation cannot be executed due to the given argument exceeding the permitted range; n > 100000. \nArguemnts exceeding this permitted range could cause your computer to become extremely slow and in \nrare cases, unstable. This safeguard was put in place to protect your system."

    fibonacci_sequence = [0, 1]
    while len(fibonacci_sequence) < n:
        next_term = fibonacci_sequence[-1] + fibonacci_sequence[-2]
        fibonacci_sequence.append(next_term)
    return fibonacci_sequence[len(fibonacci_sequence) - 1] / fibonacci_sequence[len(fibonacci_sequence) - 2]
...
def factorial(constant: int) -> int:
    try:
        ...
        return "You cannot take the factorial of a non-integer value. Use gamma function instead!"
...
def cis(x: float) -> float:
    ...
    Returns:
        float: Returns complex number using the cis method otherwise known as e^ix. The return value cannot be used in other methods.
    """
    return f"{cos(x)} + i{sin(x)}"
...
def sum(n: int=1, a: int=1):
    ...
    Returns:
        _type_: The sum of each number added together.
    """
    for i in range(n, a):
        i+=(n+1)
        n=i
...
class Table():
    ...
    def points(self):
        if len(self.L1_X) > len(self.L2_Y) or len(self.L2_Y) > len(self.L1_X):
            print("Not all values have a corresponding pair, be sure to check if all values in list one have a pair in list two and vise versa.")
        
        n = 0
        while n < len(self.L1_X):
            print(f"{(self.L1_X[n], self.L2_Y[n])}")
            n+=1

    def linear_regression(self) -> str:
        """Uses the mean (average) of all x and y values to produce a rough approximation of a linear graph. 

        Returns:
            str: Line of best fit, rough linear representation of all x and y values (Uses the form a*x=b*y±c).
        """
        c = str(mean(self.L2_Y) - (mean(self.L1_X) / mean(self.L2_Y)) * mean(self.L1_X))
        if float(c) < 0:
            return f"{mean(self.L1_X)}x = {mean(self.L2_Y)}y - {c}"
        elif float(c) > 0:
            return f"{mean(self.L1_X)}x = {mean(self.L2_Y)}y + {c}"
        elif float(c) == 0:
            return f"{mean(self.L1_X)}x = {mean(self.L2_Y)}y "
